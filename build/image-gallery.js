...
var ImageGallery = function (_React$Component) {
  ...
  function ImageGallery(props) {
    ...
    _this._initGalleryResizing = function (el) {
      _this._imageGallerySlideWrapper = el;
    ...
    _this._createResizeObserver = function (entries) {
      (0, _lodash6.default)(entries, function (entry) {
        var _entry$contentRect = entry.contentRect,
            width = _entry$contentRect.width,
            height = _entry$contentRect.height;

        _this._handleResize(width, height);
      });
    };

    _this._handleResize = function (width, height) {
      ...
      if (_this._thumbnailsWrapper) {
        if (_this._isThumbnailHorizontal()) {
          _this.setState({ thumbnailsWrapperHeight: height });
        } else {
          _this.setState({ thumbnailsWrapperWidth: width });
        }
      }
